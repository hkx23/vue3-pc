  ...
  >
    ...
    <template #footer>
      <div v-if="current == 0">
        ...
        <!-- <t-button theme="primary" :disabled="!worksheetData" @click="importSubmit">导入</t-button> -->
      </div>
      <!-- <div v-if="current == 1">
...
<script setup lang="tsx" name="BcmpImportMultipleDialog">
...
import { CommonImportMoodel, ImportSummary } from './constants';
// 首先定义一个接口来描述列配置项
...
const loadFile = (res): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e: ProgressEvent<FileReader>) => {
      ...
      // 初始化JSON数组和标题行
      const jsonData: any[] = [];
      const jsonDataTwo: any[] = [];
      const headers: string[] = [];
      const headersTwo: string[] = [];

      const sheetName = workbook.SheetNames[0]; // 一个工作表
      const sheetNameTwo = workbook.SheetNames[1]; // 处理第2个工作表
      const worksheet = workbook.Sheets[sheetName];
      const worksheetTwo = workbook.Sheets[sheetNameTwo];

      // 解析第一个工作表的数据
      const firstWorksheetData = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { header: 1 });

      // 判断第一个工作表除标题外是否只有一行数据
      if (firstWorksheetData.length > 2) {
        // 除标题外只能有一行数据
        reject(new Error('第一个工作表只能有一行数据'));
        return;
      }
      ...
      if (range.e.r > props.importRowCountLimit || rangeTwo.e.r > props.importRowCountLimit) {
        reject(new Error(`导入文件行数不能超过${props.importRowCountLimit}行`));
        return;
      }
      if (range.e.r < 1 || rangeTwo.e.r < 1) {
      ...
      for (let col = rangeTwo.s.c; col <= rangeTwo.e.c; col++) {
      ...
      for (let row = range.s.r; row <= range.e.r; row++) {
        ...
        const currentPercent = Math.round((row / (range.e.r + rangeTwo.e.r)) * 100);
      ...
      for (let row = rangeTwo.s.r; row <= rangeTwo.e.r; row++) {
        const rowTwoData: Record<string, string> = {};
        const currentPercent = Math.round(((row + jsonData.length) / (range.e.r + rangeTwo.e.r)) * 100);
        if (currentPercent !== tempPercent) {
          uploadRef.value.uploadFilePercent({ file: res, percent: currentPercent });
          tempPercent = currentPercent;
        }
        for (let col = rangeTwo.s.c; col <= rangeTwo.e.c; col++) {
          ...
          // 检查当前单元格是否在合并范围内
          for (const merge of mergesTwo) {
            const mergedRangeStart = XLSX.utils.decode_cell(merge.s.toString());

            if (mergedRangeStart.r === row && mergedRangeStart.c === col) {
              const mergedRange = XLSX.utils.decode_range(merge.toString());
              cellValue = getMergedCellValue(worksheetTwo, mergedRange);
              break;
            }
          }
          ...
          if (!cellValue) {
            const cellAddress = { r: row, c: col };
            const encodedCellAddress = XLSX.utils.encode_cell(cellAddress);
            const cell = worksheetTwo[encodedCellAddress];
            cellValue = cell ? cell.v : null;
          }
