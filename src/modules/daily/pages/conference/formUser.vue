<template>
  ...
  >
    ...
    >
      <t-form-item :label="t('conference.参与用户')" name="userIds">
...
const FORM_RULES = {
  userIds: [{ required: true, message: t('common.placeholder.select', [t('conference.参与用户')]) }],
};
...
const confirm = () => {
  return new Promise((resolve, reject) => {
    if (!formData.userIds) {
      MessagePlugin.warning(t('common.placeholder.select', [t('conference.参与用户')]));
      reject();
      return;
    }
    if (props.isAdd) {
      formRef.value.validate().then((result) => {
        ...
        formData.userIds.forEach((item) => {
          const conferenceUser = {} as any;
          conferenceUser.conferenceId = formData.conferenceId;
          conferenceUser.userId = item.value;
          conferenceUsers.push(conferenceUser);
        });
        apiDaily.conference.addUser({ conferenceUsers, conferenceId: formData.conferenceId }).then(() => {
