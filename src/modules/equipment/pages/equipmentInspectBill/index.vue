      ...
      <cmp-container :full="true">
        ...
        <cmp-card :ghost="true">
          ...
          >
            ...
            <template #userInspectName="{ row }">
              {{ row.inspectResult || '-' }}
            </template>
            <template #workshopName="{ row }">
              {{ row.workshopName || '-' }}
            </template>
            <template #datetimeInspectEnd="{ row }">
              {{ row.datetimeInspectEnd || '-' }}
            </template>
            <template #repairBillNo="{ row }">
              {{ row.repairBillNo || '-' }}
            </template>
          </cmp-table>
        </cmp-card>
...
const rowInspectBillID = ref(''); // 点击行ID
const equipmentRelateRef = ref();
const onPlanSelectChange = async ({ row }) => {
  // rowInspectBillID.value = row.id || row.ID;
  rowInspectBillID.value = row.billNo;
  relateCondition.value = [
    {
      tableName: 'm_inspect_bill_dtl',
      field: 'billNo',
      operator: 'eq',
      value: rowInspectBillID.value,
    },
  ];
  // 等待 DOM 更新完成
  await nextTick();
...
const opts = computed(() => {
  return {
    billNo: {
      label: t('inspectBill.单据编号'),
      comp: 't-input',
      defaultVal: '',
      placeholder: t('common.placeholder.input', [`${t('inspectBill.单据编号')}`]),
    },
    equipmentName: {
      label: t('inspectBill.设备名称或编码'),
      comp: 't-input',
      defaultVal: '',
      placeholder: t('common.placeholder.input', [`${t('inspectBill.设备名称或编码')}`]),
    },
    timeCreate: {
      label: t('inspectBill.点检日期范围'),
    ...
    status: {
      label: t('inspectBill.单据状态'),
      comp: 't-select',
      operator: 'eq',
      bind: {
        options: [
          // { label: '全部', value: '' },
          { label: '待点检', value: 'UNINSPECT' },
          { label: '点检中', value: 'INSPECTING' },
...
const fetchTable = async () => {
  ...
  try {
    ...
    const res: any = await api.inspectBillHead.search(searchCondition);
