        ...
        <cmp-card :ghost="true">
          ...
          >
            ...
            <template #state="{ row }">
              <t-popconfirm
                :content="
                  row.state == 0 ? t('maintenanceItem.是否确认启用项目') : t('maintenanceItem.是否确认禁用项目')
                "
...
const fetchTable = async () => {
  ...
  try {
    ...
    const res: any = await api.maintenanceItem.search(searchCondition);
...
const onFileView = async (row: any) => {
  console.log('查看', row);
  const fileList = (await api.maintenanceItemFile.getFileListByItemId(row.id)) as any;
...
const onEditRow = async (row: any) => {
  ...
  uploadPath.value = `equipment/maintenanceItem/${row.id}`;
  // 加载附件列表
  currentEditId.value = row.id;
  fileList.value = (await api.maintenanceItemFile.getFileListByItemId(row.id)) as any;
...
const onAddClick = () => {
  ...
  uploadPath.value = `equipment/maintenanceItem/${currentEditId.value}`;
...
//   await api.maintenanceItem.removeItemsById(deleteModel);
//   fetchTable();
// };
    ...
    if (result !== true) {
      ...
      try {
        if (formAction.value === 'edit') {
          // 编辑
          await api.maintenanceItem.updateItemByCode(postData);
        } else {
          // 新增
          await api.maintenanceItem.addItem(postData);
        }
        // 第二步：提交数据
...
const onRowStateChange = async (row: any) => {
  ...
  await api.maintenanceItem.updateItemByCode(postRow).then(() => {
