<template>
  <cmp-container :full="true" :full-sub-index="[0, 1]">
      ...
      <cmp-container :full="true">
        ...
        <cmp-card :ghost="true">
          ...
          >
            ...
            <template #state="{ row }">
              <t-popconfirm
                :content="row.state == 0 ? t('repairItem.是否确认启用项目') : t('repairItem.是否确认禁用项目')"
    ...
    <cmp-card>
      ...
      <relate-equipment ref="equipmentRelateRef" :ghost="true" :relate-condition="relateCondition"> </relate-equipment>
    </cmp-card>
  </cmp-container>
...
import relateEquipment from './relateEquipment.vue';
...
const uploadPath = ref('main/equipment');
const { t } = useLang();
...
const opts = computed(() => {
  return {
    keyWord: {
      label: t('repairItem.故障'),
      comp: 't-input',
      defaultVal: '',
      placeholder: t('common.placeholder.input', [`${t('repairItem.故障编码或故障描述')}`]),
    },

    assetTypeId: {
      label: t('repairItem.设备类型'),
      ...
      placeholder: t('common.placeholder.select', [`${t('repairItem.设备类型')}`]),
    },
    assetBrandId: {
      label: t('repairItem.设备品牌'),
      comp: 'bcmp-select-business',
      event: 'business',
      defaultVal: '',
      bind: {
      ...
      placeholder: t('common.placeholder.select', [`${t('repairItem.设备品牌')}`]),
    },
    assetModelId: {
      label: t('repairItem.设备型号'),
      comp: 'bcmp-select-business',
      event: 'business',
      defaultVal: '',
      bind: {
      ...
      placeholder: t('common.placeholder.select', [`${t('repairItem.设备型号')}`]),
    },
    state: {
      label: t('repairItem.只显示启用'),
      comp: 't-checkbox',
      defaultVal: '',
    },
  };
...
const fetchTable = async () => {
  ...
  try {
    ...
    const res: any = await api.repairItem.search(searchCondition);
...
const onFileView = async (row: any) => {
  console.log('查看', row);
  const fileList = (await api.repairItemFile.getFileListByItemId(row.id)) as any;
...
const onEditRow = async (row: any) => {
  ...
  uploadPath.value = `equipment/repairItem/${row.id}`;
  // 加载附件列表
  currentEditId.value = row.id;
  fileList.value = (await api.repairItemFile.getFileListByItemId(row.id)) as any;
...
const onAddClick = () => {
  ...
  uploadPath.value = `equipment/repairItem/${currentEditId.value}`;
...
//   await api.repairItem.removeItemsById(deleteModel);
//   fetchTable();
// };
    ...
    if (result !== true) {
      ...
      try {
        if (formAction.value === 'edit') {
          // 编辑
          await api.repairItem.updateItemByCode(postData);
        } else {
          // 新增
          await api.repairItem.addItem(postData);
        }
        // 第二步：提交数据
...
const onRowStateChange = async (row: any) => {
  ...
  await api.repairItem.updateItemByCode(postRow).then(() => {
...
//       try {
//         // 第二步：提交数据
//         await api.equipment.addItem(addFormRef.value.getFormData());
//         MessagePlugin.success('提交成功');
//         formVisible.value = false;
//         fetchTable();
//       } catch (e) {
