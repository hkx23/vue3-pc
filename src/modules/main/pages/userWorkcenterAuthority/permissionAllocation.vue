      ...
      <cmp-card flex="auto">
        <t-space direction="vertical" :size="8" style="padding: 0">
          <cmp-card :span="12" :ghost="true">
            ...
            >
              <template #operate>
                <t-input v-model="param.boxCode" placeholder="请输入工作中心编码/工作中心名称" :on-enter="onInputEnter">
                  <template #prefix-icon>
                    <icon name="search"></icon>
                  </template>
                </t-input>
                <t-button :loading="saveLoading" theme="primary" @click="onBtnSave">保存工作中心权限</t-button>
              </template>
            </cmp-table>
          </cmp-card>
        </t-space>
...
import { api, api as apiMain, WorkstationAuthVO } from '@/api/main';
...
const onBtnSave = debounce(async () => {
  ...
  await apiMain.userWorkcenterAuthority.saveAuthority({
    userId: userId.value,
    inseartList: addArr,
    removeList: delArr,
  });
...
const onGetAllPermission = async () => {
  ...
  const res = await api.workcenter.search(searchList.value);
  data.value = res.list;
...
const onGetTickPermission = async () => {
  const res = await apiMain.userWorkcenterAuthority.getUserAuthority({ userId: userId.value });
